...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('binhluan.sua', ['id' => $binhluan->id]) }}" method="post">
                @csrf
                <div class="mb-3">
                    ...
                    <select name="phim_id" id="phim_id"
                        ...
                        @foreach ($phim as $value)
                            <option value="{{ $value->id }}" {{ ($binhluan->phim_id == $value->id) ? 'selected' : '' }}>
                                {{ $value->tenphim }}
                            </option>
                        @endforeach
                        @error('phim_id')
                ...
                <div class="mb-3">
                    <label class="form-label" for="khachhang_id">Khách Hàng</label>
                    <select name="khachhang_id" id="khachhang_id"
                        class="form-control @error('khachhang_id') is-invalid @enderror" required>
                        <option value="">--Chọn--</option>
                        @foreach ($khachhang as $value)
                            <option value="{{ $value->id }}"  {{ ($binhluan->khachhang_id == $value->id) ? 'selected' : '' }}>
                                {{ $value->name }}
                            </option>
                        @endforeach
                        @error('khachhang_id')
